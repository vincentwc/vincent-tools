...
public final class VincentAdminSpaHtml {
    private VincentAdminSpaHtml() {
    }

    public static String inject(String html, String configGlobalName, String apiPath, String historyBase) {
        ...
        String injectedHead = "<base href=\"" + htmlAttribute(normalizeBaseHref(normalizedHistoryBase)) + "\">"
                + "<script>window." + configGlobalName + "={"
    ...
    public static byte[] injectBytes(Resource resource, String configGlobalName, String apiPath, String historyBase)
            throws IOException {
        return inject(read(resource), configGlobalName, apiPath, historyBase).getBytes(StandardCharsets.UTF_8);
    }
